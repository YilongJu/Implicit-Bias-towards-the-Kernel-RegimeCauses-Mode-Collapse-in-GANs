...
def Get_grad_norm_plot_ready_df(result_tuned_df, final_iter=400000):
    ...
    result_tuned_clean_melted = pd.melt(result_tuned_clean_unmelt,
                                        id_vars=["data", r"$H$", r"$\gamma$", r'$\eta$', 'prop_neg_samples', 'log_KL', 'log_KL_mode', "loss_D",
                                                 "covered_mode_num", 'update_angle_BP_directions_G_tot_deg_mean', 'update_BP_distances_G_tot_norm_mean',
                                                 'update_BP_delta_slopes_G_tot_norm_mean',
...
def Adding_init_and_final_values_to_result_df(results_df):
    ...
    results_df_cleaned = results_df.copy()
    results_df_cleaned.loc[results_df_cleaned["opt_type"] == "sgd", "gamma"] = 1
    results_df_cleaned = Fix_float_error_for_df(results_df_cleaned, "g_lr", [0.0000001, 0.000000316, 0.000001, 0.00000316, 0.00001, 0.0000316, 0.0001, 0.000316, 0.001, 0.00316, 0.01])
    results_df_cleaned = Fix_float_error_for_df(results_df_cleaned, "d_lr", [0.0000001, 0.000000316, 0.000001, 0.00000316, 0.00001, 0.0000316, 0.0001, 0.000316, 0.001, 0.00316, 0.01])
    results_df_cleaned = Fix_float_error_for_df(results_df_cleaned, "alpha_mobility", [0.0000001, 0.000000316, 0.000001, 0.00000316, 0.00001, 0.0000316, 0.0001, 0.000316, 0.001, 0.00316, 0.01, 0.0316, 0.1, 0.316, 1, 3.16, 10, 31.6, 100])
    results_df_cleaned = Fix_float_error_for_df(results_df_cleaned, "alpha_mobility_D", [0.0000001, 0.000000316, 0.000001, 0.00000316, 0.00001, 0.0000316, 0.0001, 0.000316, 0.001, 0.00316, 0.01, 0.0316, 0.1, 0.316, 1, 3.16, 10, 31.6, 100])
    results_df_cleaned = Fix_float_error_for_df(results_df_cleaned, "gamma", [0.8, 0.9, 0.99, 0.999, 0.9999, 1])
    single_value_column_list = [column for column in results_df_cleaned.columns if len(Get_nonnan_unique_values(results_df_cleaned[column].unique())) <= 1] + ["opt_type"]
    ...
    results_df_cleaned = results_df_cleaned.drop(columns=["BP_density_range_dim_1", "BP_density_range_dim_2", "BP_G_1_entropy", "BP_G_2_entropy", "zeta"])
    var_with_init_or_final_list = [column for column in results_df_cleaned.columns if "init" in column or "final" in column or "update" in column or "diff" in column]
    ...
    results_df_cleaned = pd.merge(results_df_cleaned_leave_out_vars[results_df_cleaned_leave_out_vars["iter"] == final_iter], results_df_cleaned_needs_init_vals_added, on=exp_setting_var_list)
...
if __name__ == "__main__":
    load_deepIvEst_3_2 = True

    result_csv_task_dir_list = ["Grid_simgd_gamma0.8-0.9-0.99-0.999-0.9999-1_widthGD-32-64-128-256-512-1024-2048",
                                "Grid_simgd_gamma0.8-0.9-0.99-0.999-0.9999-1_widthGD-4096-8192-16384"]
    results_df = Get_result_df_from_result_csv_task_dir_list(result_csv_task_dir_list)
    results_df, results_df_cleaned, results_fe_df = Adding_init_and_final_values_to_result_df(results_df)
    results_fe_df_normalized = results_fe_df.copy()
    results_fe_df_normalized = results_fe_df_normalized[results_fe_df_normalized["log_KL"] < 3.18] ## log KL greater than this value correspond to training divergence.
    results_fe_df_normalized = (results_fe_df_normalized - results_fe_df_normalized.mean()) / results_fe_df_normalized.std()
    results_fe_df_normalized["intercept"] = 1

    treatments = ["update_angle_BP_directions_G_tot_deg", "BP_delta_slopes_G_L1_norm_final", "biases_G_norm_final", "dG_dz_mean_final", "rel_act_diff_G",
                  "ntk_G_centered_effective_rank_final"]
    controls = ["loss_G_final", "ntk_D_centered_effective_rank_final", "dD_dx_mean_final", "BP_delta_slopes_D_L1_norm_final", "update_biases_D_norm_all"]
    instruments = ["dG_dz_mean_init", "BP_delta_slopes_G_L1_norm_init", "biases_G_norm_init", "weights_G_norm_init", "vweights_G_norm_init",
                   "ntk_G_centered_effective_rank_init", "BP_distances_G_mean_init"]
    K.clear_session()
    base_width = 64
    treatment_model = keras.Sequential([keras.layers.Dense(base_width * 4, activation='relu', input_shape=(len(instruments) + len(controls),)),
                                        keras.layers.BatchNormalization(),
                                        keras.layers.Dense(base_width * 2, activation='relu'),
                                        keras.layers.BatchNormalization(),
                                        keras.layers.Dense(base_width, activation='relu')])

    response_model = keras.Sequential([keras.layers.Dense(base_width * 4, activation='relu', input_shape=(len(treatments) + len(controls),)),
                                       keras.layers.BatchNormalization(),
                                       keras.layers.Dense(base_width * 2, activation='relu'),
                                       keras.layers.BatchNormalization(),
                                       keras.layers.Dense(base_width, activation='relu'),
    ...
    first_stage_options = {"epochs": 15,
                           "validation_split": 0.1,
                           "callbacks": [keras.callbacks.EarlyStopping(patience=5, restore_best_weights=True)]}

    second_stage_options = {"epochs": 30,
                            "validation_split": 0.2,
                            "callbacks": [keras.callbacks.EarlyStopping(patience=5, restore_best_weights=True)]}



    if load_deepIvEst_3_2:
        deepIvEst_3_2_dict = pickle5.load(open("Causal/deepIvEst_3_2.dict", "rb"))
        marginal_effect = deepIvEst_3_2_dict["marginal_effect_df"][[f"{treatment}_meff" for treatment in deepIvEst_3_2_dict["treatments"]]].values
    else:
        deepIvEst_3_2 = DeepIV(n_components=20,  # number of gaussians in our mixture density network
                               ...
                               optimizer=keras.optimizers.Adam(learning_rate=0.001),  # Keras optimizer to use for training - see https://keras.io/optimizers/
                               first_stage_options=first_stage_options,  # options for training treatment model
                               second_stage_options=second_stage_options)  # options for training response model
        deepIvEst_3_2.fit(Y=results_fe_df_normalized["log_KL"].values, T=results_fe_df_normalized[treatments].values,
                          X=results_fe_df_normalized[controls].values,
                          Z=results_fe_df_normalized[instruments].values)

        deepIV_model = deepIvEst_3_2
        model_name = "deepIvEst_3_2"
        Y_predicted = deepIV_model.predict(T=results_fe_df_normalized[treatments].values, X=results_fe_df_normalized[controls].values)
        plt.figure(figsize=(10, 10))
        plt.scatter(results_fe_df_normalized["log_KL"].values, Y_predicted)
        plt.plot(np.linspace(np.min(results_fe_df_normalized["log_KL"].values), np.max(results_fe_df_normalized["log_KL"].values), 2),
                 np.linspace(np.min(results_fe_df_normalized["log_KL"].values), np.max(results_fe_df_normalized["log_KL"].values), 2), color="#000000")
        plt.gca().set_aspect("equal")

        TSS = np.sum((results_fe_df_normalized["log_KL"].values - np.mean(results_fe_df_normalized["log_KL"].values)) ** 2)
        RSS = np.sum((results_fe_df_normalized["log_KL"].values - Y_predicted) ** 2)
        R_squared = 1 - RSS / TSS
        plt.title(f"R squared = {R_squared:.4f}")
        plt.xlabel("Actual values")
        plt.ylabel("Predicted values")
        plt.show()
        marginal_effect = deepIV_model.marginal_effect(T=results_fe_df_normalized[treatments].values, X=results_fe_df_normalized[controls].values)

    treatment_dict = {"update_angle_BP_directions_G_tot_deg": r"$\sum_t |\Delta_t \xi_G|$",
                      "BP_delta_slopes_G_L1_norm_final": r"$||\mu_G^{(T)}||_1$",
                      "biases_G_norm_final": r"$||b_G^{(T)}||_2$",
                      "dG_dz_mean_final": r"$\mathbb{E}_{z\sim \mathcal{N}(0, I)} [||\nabla_z G^{(T)}||_F]$",
                      "rel_act_diff_G": r"$|\Delta \mathrm{Act}_G \%|$",
                      "ntk_G_centered_effective_rank_final": r"$\mathrm{rank}_{\mathrm{eff}}(\mathrm{NTK}_G)$"}
    print(treatment_dict)
    sns.set(font_scale=2)
    base_figsize = 5
    aspect_ratio = 1
    plt.subplots(1, marginal_effect.shape[1], figsize=(base_figsize * marginal_effect.shape[1], base_figsize / aspect_ratio))
    for j in range(marginal_effect.shape[1]):
        plt.subplot(1, marginal_effect.shape[1], j + 1)
        plt.hist(marginal_effect[:, j], bins=30, density=True)
        effect_mean = np.mean(marginal_effect[:, j])
        plt.gca().axvline(effect_mean, color="#FF0000")
        plt.xlabel(f"{treatment_dict[treatments[j]]}")
        if j != 0:
