...
class VisionData():
    def __init__(self, D, G, device, dataset, z_dim=8, batchsize=256, lr=0.1, show_iter=100,
                 weight_decay=0.0,
                 d_penalty=0.0, g_penalty=0.0, noise_shape=(64, 8), gp_weight=10, gpu_num=1):
        self.lr = lr
    ...
    def plot_grad(self, gd, gg, hd=None, hg=None, cg_d=None, cg_g=None):
        ...
        # self.writer.add_scalars('Delta', {'D gradient': gd, 'G gradient': gg}, self.count)
    ...
    def train_gd(self, epoch_num, mode='Adam', dataname='MNIST', logname='MNIST', loss_type='JSD'):
        ...
        self.writer_init(logname=logname,
                         comments='%s-%.3f_%.5f' % (mode, self.lr, self.weight_decay))
            ...
            for real_x in self.dataloader:
                ...
                if self.count % 100 == 0:
                    ...
                    self.iswriter.writerow(
                        {'iter': self.count, 'is_mean': is_mean, 'is_std': is_std,
                         'time': time.time() - start})
                    self.save_checkpoint('%s-%.5f_%d.pth' % (mode, self.lr, self.count),
                                         dataset=dataname)
        self.writer.close()
        self.save_checkpoint('DIM64%s-%.5f_%d.pth' % (mode, self.lr, self.count), dataset=dataname)

    def train_bcgd(self, epoch_num=100, mode='BCGD', collect_info=False, dataname='CIFAR',
                   logname='CIFAR', loss_type='JSD'):
        timer = time.time()
        start = time.time()
        is_lsit = []
        if collect_info:
            ...
            self.iswriter.writeheader()
        if mode == 'BCGD':
            optimizer = BCGD(max_params=self.G.parameters(), min_params=self.D.parameters(), lr_max=self.lr, lr_min=self.lr, device=self.device, solve_x=False, collect_info=collect_info)
        elif mode == 'ACGD':
            optimizer = ACGD(max_params=self.G.parameters(), min_params=self.D.parameters(), lr_max=self.lr, lr_min=self.lr, device=self.device, solve_x=False, collect_info=collect_info)
        ...
        for e in range(epoch_num):
            for real_x in self.dataloader:
                ...
                if collect_info:
                    grad_x, grad_y, hg, hd, cg_g, cg_d, time_cg, iter_num = optimizer.get_info()
                    self.plot_param(D_loss=loss, total_loss=lossp)
                    self.plot_grad(gd=grad_y, gg=grad_x, hd=hd, hg=hg, cg_d=cg_d, cg_g=cg_g)
...
def train_wgan():
    ...
    trainer.train_bcgd(epoch_num=5001, mode='ACGD', collect_info=True, dataname='CIFAR10-WGAN',
