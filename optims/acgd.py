...
class ACGD(object):
    ...
    def get_info(self):
        ...
        return self.info['grad_x'], self.info['grad_y'], self.info['hvp_x'], self.info['hvp_y'], self.info['cg_x'], self.info['cg_y'], self.info['time'], self.info['iter_num']
    ...
    def step(self, loss):
        ...
        grad_x_vec = torch.cat([g.contiguous().view(-1) for g in grad_x]) # Dx_g
        grad_y = autograd.grad(loss, self.min_params, create_graph=True, retain_graph=True)
        grad_y_vec = torch.cat([g.contiguous().view(-1) for g in grad_y]) # Dy_g
        grad_x_vec_d = grad_x_vec.clone().detach() # Dx_g
        ...
        lr_max = math.sqrt(bias_correction) * lr_max / sq_avg_x.sqrt().add(eps) # lr_x
        lr_min = math.sqrt(bias_correction) * lr_min / sq_avg_y.sqrt().add(eps) # lr_y
        ...
        hvp_x_vec = Hvp_vec(grad_y_vec, self.max_params, scaled_grad_y,
                            retain_graph=True)  # h_xy * d_y --> Hxy_g_Dy_g
        hvp_y_vec = Hvp_vec(grad_x_vec, self.min_params, scaled_grad_x,
                            retain_graph=True)  # h_yx * d_x --> Hyx_g_Dx_g
        p_x = torch.add(grad_x_vec_d, - hvp_x_vec) # Dx_g - lr_y * Hxy_g_Dy_g
        p_y = torch.add(grad_y_vec_d, hvp_y_vec)   # Dy_g + lr_x * Hyx_g_Dx_g
        ...
        if self.state['solve_x']:
            p_y.mul_(lr_min.sqrt()) # (Dy_g + lr_x * Hyx_g_Dx_g) * sqrt(lr_y)
            cg_y, iter_num = general_conjugate_gradient(grad_x=grad_y_vec, grad_y=grad_x_vec,
                                                        ...
                                                        tol=tol, atol=atol,
                                                        lr_x=lr_min, lr_y=lr_max, device=self.device) # inv(I + sqrt(lr_y) * Hyx_g * lr_x * Hxy_g * sqrt(lr_y)) * (Dy_g + lr_x * Hyx_g_Dx_g) * sqrt(lr_y)
            old_min = cg_y.detach_()
            min_update = cg_y.mul(-lr_min.sqrt()) # -lr_y * inv(I + sqrt(lr_y) * Hyx_g * lr_x * Hxy_g * sqrt(lr_y)) * (Dy_g + lr_x * Hyx_g_Dx_g) = -lr_y * CDy_g
            hcg = Hvp_vec(grad_y_vec, self.max_params, min_update).detach_() # -lr_y_Hxy_g_CDy_g
            hcg.add_(grad_x_vec_d) # Dx_g - lr_y_Hxy_g_CDy_g
            max_update = hcg.mul(lr_max) # lr_x * (Dx_g - lr_y_Hxy_g_CDy_g)
            old_max = hcg.mul(lr_max.sqrt()) # sqrt(lr_x) * (Dx_g - lr_y_Hxy_g_CDy_g)
        else:
